...
import com.howtographql.scala.demo.models.{AuthenticationException, AuthorizationException, User}
...
case class MyContext(dao: DAO, currentUser: Option[User] = None) {
  ...
  def ensureAuthenticated() = {
    if (currentUser.isEmpty) {
      throw AuthorizationException("You do not have permission ,please login")
    }
  }
}
